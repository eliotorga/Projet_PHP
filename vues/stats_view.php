    ...
    <div class="page-container">
        ...
        <div class="table-container">
            ...
            <div class="filters-container">
                <!-- Formulaire de filtrage via PHP -->
                <form method="GET" action="" class="filters-form">
                    <div class="filter-group">
                        ...
                        <select name="statut" class="filter-select">
                            ...
                            <?php foreach ($joueurs_statut as $stat): ?>
                                <option value="<?= $stat['statut_code'] ?>" <?= isset($_GET['statut']) && $_GET['statut'] == $stat['statut_code'] ? 'selected' : '' ?>>
                    ...
                    <div class="filter-group">
                        ...
                        <input type="text" name="recherche" class="search-input" placeholder="Nom, prénom..." value="<?= htmlspecialchars($_GET['recherche'] ?? '') ?>">
                    </div>
                    
                    <div class="filter-group">
                        <label class="filter-label">Trier par</label>
                        <select name="tri" class="filter-select">
                            <option value="nom" <?= ($_GET['tri'] ?? 'nom') == 'nom' ? 'selected' : '' ?>>Nom A-Z</option>
                            <option value="impact_desc" <?= ($_GET['tri'] ?? '') == 'impact_desc' ? 'selected' : '' ?>>Score d'impact ▼</option>
                            <option value="impact_asc" <?= ($_GET['tri'] ?? '') == 'impact_asc' ? 'selected' : '' ?>>Score d'impact ▲</option>
                            <option value="moyenne_desc" <?= ($_GET['tri'] ?? '') == 'moyenne_desc' ? 'selected' : '' ?>>Note décroissante</option>
                            <option value="victoires_desc" <?= ($_GET['tri'] ?? '') == 'victoires_desc' ? 'selected' : '' ?>>% victoires</option>
                        </select>
                    </div>
                    
                    <div class="filter-group">
                        <button type="submit" class="filter-select" style="background: var(--primary); color: white; cursor: pointer;">
            ...
            <table class="stats-table">
                ...
                <tbody>
                    ...
                    if (!empty($_GET['statut'])) {
                        $joueurs_filtres = array_filter($joueurs_filtres, function($j) {
                            return $j['statut_code'] == $_GET['statut'];
                        });
                    }
                    
                    if (!empty($_GET['recherche'])) {
                        $recherche = strtolower($_GET['recherche']);
                        $joueurs_filtres = array_filter($joueurs_filtres, function($j) use ($recherche) {
                            return stripos($j['nom'] . ' ' . $j['prenom'], $recherche) !== false;
                        });
                    }
                    
                    // Tri côté serveur
                    if (!empty($_GET['tri'])) {
                        usort($joueurs_filtres, function($a, $b) {
                            switch ($_GET['tri']) {
                                case 'impact_desc':
                                ...
                                case 'moyenne_desc':
                                    return ($b['moyenne_notes'] ?? 0) <=> ($a['moyenne_notes'] ?? 0);
                                case 'victoires_desc':
